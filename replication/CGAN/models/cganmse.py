...
class Cgan:
    def __init__(self, config, div_estimator=False):
        ...
        self.rmse_loss = nn.MSELoss(reduction="mean")
        ...
        for epoch_i in range(config["epochs"]):
            ...
            for batch_i, (x_batch, data_batch) in enumerate(train_loader):
                ...
                if config["gen_samples"] > 1:
                    ...
                    data_batch_repeated = torch.repeat_interleave(data_batch,
                            config["gen_samples"], dim=0)
                else:
                ...
                gen_loss = self.gen_loss(data_batch_repeated,new_gen_batch)
